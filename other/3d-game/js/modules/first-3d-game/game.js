...
function createScene() {
	...
	ball.position.x = 0;
	ball.position.y = 0;
	// set the ball above the table surface
	ball.position.z = radius;
	ball.receiveShadow = true;
	ball.castShadow = true;
	
	// set up the paddle variables
	paddleWidth = 10;
	paddleHeight = 30;
	paddleDepth = 10;
	paddleQuality = 1;
	...
	// iterate 10x (5x each side) to create pillars to show off shadows
	...
	// finish by adding a ground plane to show off pretty shadows
	...
	// add a spot light for casting shadows
	...
	// magic shadow creator deluxe edition with lights pack
...
function opponentPaddleMovement() {
	...
	{
		// if paddle is lerping in the +ve direction
...
function playerPaddleMovement() {
	...
	{
		// if paddle is not touching the side of the table
		// we move
		if (paddle1.position.y > -fieldHeight * 0.45)
		{
			paddle1DirY = -paddleSpeed * 0.5;
	...
	if (ball.position.x <= paddle1.position.x + paddleWidth && ball.position.x >= paddle1.position.x)
	{
		...
		if (ball.position.y <= paddle1.position.y + paddleHeight/2 && ball.position.y >= paddle1.position.y - paddleHeight/2)
		{
			...
			if (ballDirX < 0)
			{
				...
				// this is not realistic physics, just spices up the game
...
function resetBall(loser) {
	...
	// if the player lost the last point, we send the ball to the opponent
	if (loser == 1)
	{
		ballDirX = -1;
	}
